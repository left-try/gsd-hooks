...
- [ ] **TEST-01**: `gsd-gemini-after` runtime integration test verifies 429 transcript triggers economy.lock (parity with `gsd-codex-429-guard.test.js`)
- [ ] **DOCS-02**: `test/readme.test.js` asserts `gsd-gemini-after` is documented in README

### Verification Debt

- [ ] **VERIFY-01**: Formal `05-VERIFICATION.md` retroactively documents Phase 5 multi-runtime success criteria with test evidence

### Publish Polish

- [ ] **PUB-01**: `package.json` publish metadata committed (`files` whitelist, `repository` field) — no `.planning/` in npm tarball
- [ ] **PUB-02**: Version bumped to `0.1.1` with README install pin updated
- [ ] **PUB-03**: README documents GitHub install fallback (`npx github:left-try/gsd-hooks`) for users without npm access
